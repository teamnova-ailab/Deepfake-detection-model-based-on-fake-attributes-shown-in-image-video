...
# 입력 데이터 변환 - img 모델
def final_convert_input_data(final_sentences):
    # BERT의 토크나이저로 문장을 토큰으로 분리
...
# 라벨+퍼센트 출력 - img 모델
def final_test_sentences(final_sentences):
    inputs, masks = final_convert_input_data(final_sentences)
    b_input_ids = inputs.to(device)
    b_input_mask = masks.to(device)
    with torch.no_grad():
...
for img_file in img_list :
    ...
    label_add = '1st filter ' + str(fst_logits) + ' 3rd filter ' + str(trd_logits)  + ', deepfake filter ' + str(deepfake_logits)
         ...
         # 이미지 rnn 모델로 퍼센트 출력
    final_logits = final_test_sentences([label_add])
