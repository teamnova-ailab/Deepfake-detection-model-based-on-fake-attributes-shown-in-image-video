...
def draw_bounding_box_on_image(image,
  ...
  if use_normalized_coordinates:
    # (left, right, top, bottom) = (xmin * im_width * 0.975, xmax * im_width * 1.025,
    #                               ymin * im_height * 0.875, ymax * im_height * 1.125)
  ...
  for display_str in display_str_list[0:0:-1]:
...
def draw_mask_on_image_array(image, mask, color='red', alpha=0.7):
  ...
  Args:
    ...
    mask: a float numpy array of shape (img_height, img_height) with
      values between 0 and fix_keep
    color: color to draw the keypoints with. Default is red.
    alpha: transparency value between 0 and fix_keep. (default: 0.7)
  ...
  for box, color in box_to_color_map.items():
    ...
    if keypoints is not None:
      draw_keypoints_on_image_array(
          ...
          use_normalized_coordinates=use_normalized_coordinates)
  return left, right, top, bottom, label_str